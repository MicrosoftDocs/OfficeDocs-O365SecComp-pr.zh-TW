...
現在，您機密資訊的資料庫結構描述已定義，下一個步驟是設定規則套件。 繼續前往[設定規則套件](#set-up-a-rule-package)這一節。

#### <a name="editing-the-schema-for-edm-based-classification"></a>編輯 EDM 型分類的結構描述 

(如有需要) 如果您想要變更 edm.xml 檔案，例如變更哪些欄位用於 EDM 型分類，請遵循下列步驟：

1. 編輯您的 edm.mxl 檔案 (這是本文[定義結構描述](#define-the-schema-for-your-database-of-sensitive-information)這一節所討論的檔案)。
...
#### <a name="removing-the-schema-for-edm-based-classification"></a>移除 EDM 型分類的結構描述
...
2. 執行下列 PowerShell Cmdlet，將 “patientrecords” 的資料儲存區名稱取代為您要刪除的名稱：
...
1. 以 .xml 格式建立規則套件 (使用 Unicode 編碼方式)，類似下列範例。 (您可以複製、修改及使用我們的範例。) 
   ...
   ```
    ...
    - 分類值會參照現有或自訂機密資訊類型：**classification = "U.S. Social Security Number (SSN)"**。 (在此案例中，我們使用美國社會安全號碼作為現有的機密資訊類型)。

    當您設定規則套件時，請確定正確地參照您的 .csv 檔案和 edm.xml 檔案。 (您可以複製、修改及使用我們的範例。) 
