...
工程變更後，Microsoft 支援服務會傳送給您評估的部署的日期。 請注意，在部署程序通常需要 2-3 週後您提交支援要求。 
...
2. 在 [**搜尋**] 頁面上，按一下 [![加入圖示](media/8ee52980-254b-440b-99a2-18d068de62d3.gif)**新的搜尋**。
    ...
    如先前所述，**內部部署使用者的加入 Office 應用程式內容**] 核取方塊會顯示在 [**位置**] 下。 請注意其依預設會選取。
...
5. 儲存並執行搜尋。 從內部部署使用者的雲端式信箱的任何搜尋結果可以像任何其他的搜尋結果預覽。 此外，您可以您可以將 （包括任何小組聊天資料） 的搜尋結果匯出至 PST 檔案。 如需詳細資訊，請參閱： 
...
2. 執行下列 PowerShell 命令，以建立新的內容搜尋內部部署使用者的雲端式信箱。
   ...
   建立新的搜尋之後，請務必使用**Start-compliancesearch** cmdlet 來執行搜尋。 
 ...
 **是否有任何非提交支援要求的其他需求？**
 ...
 如先前所述，具有內部部署信箱的使用者的身分識別必須同步處理至雲端式組織，讓每個內部部署使用者帳戶在 Office 365 中建立對應的郵件使用者帳戶。 此外，您的組織必須擁有 Office 365 企業版訂閱，例如 Office 365 企業版 E1、 E3 或 E5 訂閱。 
