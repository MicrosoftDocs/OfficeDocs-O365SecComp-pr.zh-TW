...
> > [!TIP]
> 選用資訊可協助使用者可更成功考量啟用在測試模式中的您 ASF 選項以最大化垃圾郵件封鎖根據您的環境。高的垃圾郵件百分比 （英文） 特定 ASF 選項的客戶，我們建議您測試先在實際執行環境之前實作它們這些選項。> 建議擔心網路釣魚的組織開啟**SPF 記錄： 完全未通過**] 選項。 
...
> （內部部署和雲端託管信箱） 這兩種情況很也不需要啟用此設定外寄郵件傳送到服務] 的合法彈跳訊息將會自動偵測到並傳遞給原始寄件者的 Ndr.> 無法使用此選項測試模式。<br/><br/>> [!TIP]
