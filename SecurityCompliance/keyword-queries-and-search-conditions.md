...
本主題說明您可以在 Exchange Online 和儲存在 SharePoint 文件中的電子郵件項目中搜尋的電子郵件和文件屬性和商務用 OneDrive 網站使用內容搜尋功能在 Office 365 安全性&amp;合規性置中。 您也可以使用**\*-ComplianceSearch**安全性中的指令程式&amp;合規性中心 PowerShell 來搜尋這些屬性。 主題亦說明：   
...
> [!NOTE]
> 內容安全性搜尋&amp;規範中心和對應的**\*-ComplianceSearch**安全性中的指令程式&amp;合規性中心 PowerShell 中使用關鍵字查詢語言 (KQL)。 如需詳細資訊，請參閱[關鍵字查詢語言語法參照](https://go.microsoft.com/fwlink/?LinkId=269603)。 
  
## <a name="searchable-email-properties"></a>可搜尋的電子郵件屬性

下表列出可搜尋的安全性使用內容搜尋功能的電子郵件內容&amp;合規性中心，或使用**New-compliancesearch**或**Set-compliancesearch** cmdlet。 該表格包含的每個屬性，以及說明範例所傳回的搜尋結果的_屬性： value_語法範例。 您可以輸入這些`property:value`內容搜尋方塊中的關鍵字組。 
...
|Bcc|電子郵件的密件副本] 欄位。<sup>1</sup>|`bcc:pilarp@contoso.com`  <br/> `bcc:pilarp`  <br/> `bcc:"Pilar Pinilla"`|所有範例會都傳回具有 [密件副本] 欄位中包含的 Pilar Pinilla 的郵件。|
...
|Cc|電子郵件 [副本] 欄位。<sup>1</sup>|`cc:pilarp@contoso.com`  <br/> `cc:"Pilar Pinilla"`|在這兩個範例中，[副本] 欄位中指定 Pilar Pinilla 的郵件。|
...
|參與者|電子郵件; 中的所有人員欄位這些欄位為、 To、 [副本] 及 [密件副本]。<sup>1</sup>|`participants:garthf@contoso.com`  <br/> `participants:contoso.com`|郵件所傳送或傳送至 garthf@contoso.com。 第二個範例會傳回所有郵件所傳送或傳送給 contoso.com 網域中的使用者。|
|已收到|收件者收到電子郵件的日期。|`received:04/15/2016`  <br/> `received>=01/01/2016 AND received<=03/31/2016`|2016 年 4 月 15 日所收到的郵件。 第二個範例會傳回 2016 年 1 月 1 日和 2016 年 3 月 31 日之間接收到的所有郵件。|
|收件者|電子郵件; 中的所有收件者欄位這些欄位是以 [副本] 及 [密件副本]。<sup>1</sup>|`recipients:garthf@contoso.com`  <br/> `recipients:contoso.com`|傳送至 garthf@contoso.com 的郵件。 第二個範例會傳回傳送給 contoso.com 網域中任何收件者的郵件。|
...
下表列出 SharePoint 和 OneDrive 的一些可搜尋的安全性使用內容搜尋功能的商務屬性&amp;合規性中心或使用**New-compliancesearch** 、 **Set-compliancesearch**指令程式。 該表格包含的每個屬性，以及說明範例所傳回的搜尋結果的_屬性： value_語法範例。 
...
|:|屬性值：|冒號 （:）在 [`property:value`語法會指定要搜尋的屬性的值包含指定的值。 例如， `recipients:garthf@contoso.com` ，則傳回 garthf@contoso.com 寄送給任何訊息。|
|=|屬性 = 值|: **:** 運算子相同。|
...
您也可以使用 「 內容搜尋 」 功能安全性&amp;合規性中心，以搜尋儲存在 SharePoint 和 OneDrive for Business 網站已與組織外部人員共用文件。 這可協助您識別共用組織外部的敏感或機密資訊。 您可以使用`ViewableByExternalUsers`關鍵字查詢中的屬性。 此屬性會傳回文件或已是使用下列其中一個下列共用方法與外部使用者共用的網站： 
...
- 當您搜尋收件者屬性，例如，從 [副本] 或 [收件者，您可以使用的 SMTP 地址別名，或顯示以註解收件者的名稱。 例如，您可以使用 pilarp@contoso.com、 pilarp 或 「 Pilar Pinilla 」。
