...
2. 執行 [**反垃圾郵件設定**] 頁面上的下列其中一個動作： 
      ...
      - 按一下 [**自訂**] 索引標籤上，變更的**自訂設定**選取器**上**，並按一下 [* *![新增圖示](media/ITPro-EAC-AddIcon.gif)才可建立新的自訂垃圾郵件篩選原則可套用至使用者群組**建立原則**] 按鈕與您組織中的網域。您也可以按兩下它們來編輯現有的自訂原則。 
    
3. 僅限自訂原則，指定原則的名稱。或者，您也可以指定詳細的說明。您不能重新命名的預設原則。<br/><br/>附註： 當您建立原則時，在單一畫面上會出現的所有組態設定。相反地，當您編輯原則，您必須瀏覽到多個畫面。設定都是相同的兩種情況，但此程序的其餘部分說明如何編輯原則時存取這些設定。 
...
4. 在 **[垃圾郵件和大量電子郵件動作]** 頁面上，在 **[垃圾郵件]** 和 **[高信賴度垃圾郵件]** 底下，選取要為內送的垃圾郵件和大量電子郵件採取的動作。根據預設，會選取 **[將郵件移至垃圾郵件資料夾]**。其他可能的值為： 
      ...
      - **將郵件移至 [垃圾郵件] 資料夾：** 將郵件傳送至指定的收件者的垃圾郵件] 資料夾。這是這兩個信賴臨界值層級的預設動作。<br/><br/>**重要： Exchange Online Protection (EOP) 客戶的： 若要讓此巨集指令來搭配內部部署信箱，您必須設定兩個 Exchange 傳輸規則來偵測垃圾郵件標頭新增 EOP 的內部部署伺服器上。如需詳細資訊，請參閱[確定垃圾郵件會路由傳送至每位使用者的垃圾郵件] 資料夾](ensure-that-spam-is-routed-to-each-user-s-junk-email-folder.md)。**
      ...
      - **前面加上文字的主旨行：** 將郵件傳送給預定的收件者，但是前面加上**前置詞與此文字的主旨行**輸入方塊中指定的文字具有的主旨行。使用此文字做為識別碼，您可以選擇性地建立篩選或路由傳送訊息所需的規則。 
        
      - **將郵件重新導向至電子郵件地址：** 將郵件傳送給預定的收件者而不是指定的電子郵件地址。**重新導向至這個電子郵件地址**輸入方塊中指定的"重新導向 」 地址。<br/><br/>附註： 如需垃圾郵件信賴等級的詳細資訊，請參閱[Spam confidence levels](spam-confidence-levels.md)。 
