...
---
# <a name="use-the-relevance-module-to-analyze-data-in-advanced-ediscovery-preview"></a>使用相關性模組來分析資料，在 [進階電子文件 （預覽）

在 [進階電子文件 （預覽），相關性模組包含相關性訓練與 < review of 與案例相關的檔案。 若要使用的相關性工作流程，前往工作集內的管理工作集，然後按一下顯示的相關性。 有幾個完成之前便可以開始工作流程所需的步驟：
- 程序： 新增至工作集每個負載集將顯示為 「 容器 」 以下。 您需要先將其新增至相關性模組; 處理這些文件這也是您可以將它們標示為植入或預先標記特定的問題。
...
- **循環的訓練與追蹤**
  ...
  - **標記**： 進階電子文件 （預覽） 可學習相關性準則專屬於根據專家的反覆運算檢閱每一項問題和標記的個別的檔案。
...
- **測試**： 可以測試結果，以確認的進階電子文件 （預覽） 處理效率的有效性。
...
- **標記和訓練**： 
  ...
  - 使用 [**略過標記**] 選項僅在必要時。 進階電子文件 （預覽） 不會訓練根據略過的檔案。 在評估，如果很難判斷檔案是否相關，最好是標記為 Relevant (R) 或不相關 （編號） 儘而不是選取 [**略過**。 當進階電子文件 （預覽） 評估訓練時，然後才能看到它如何妥善處理這些類型的檔案。
