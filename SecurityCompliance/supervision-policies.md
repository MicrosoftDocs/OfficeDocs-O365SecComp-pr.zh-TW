...
如果您輸入多個條件，Office 365 使用的所有條件一起來決定何時要將原則套用至通訊項目。 當您設定多個條件時，它們必須全部符合將原則套用，除非您輸入的例外狀況。 例如，您需要建立訊息包含 「 貿易 」 這個字，且大於 2 MB，應該套用的原則。 不過，如果郵件也會包含文字 「 Contoso 財務由已核准 」，應該不會套用原則。 因此，在此例中的三個條件就是，如下所示：
...
如果您想要減少要檢閱的內容量，您可以指定百分比由監督原則的所有通訊。 隨機選取的內容量會選取從符合您所選擇的條件的總百分比。 如果您想要檢閱者的所有項目，您可以輸入監督原則中的**100%** 。
...
若要使用的監督原則儀表板是最簡單的方法來管理監督原則結果，以及解決待處理項目。 這個儀表板可以讓檢閱者檢視項目需要檢閱，採取動作的項目，並檢閱每個監督原則的先前已檢閱並解析項目的結果。 您可以存取合規性中心，在**監督**監督原則儀表板 > *您的自訂原則* > **開啟**。
...
若要檢閱的監督原則所識別的通訊，檢閱者也可以適用於 Microsoft Outlook 的監督增益集使用。 不過，檢閱者必須執行一些步驟，以將它安裝在電腦版 Outlook 透過。 如需監督增益集以進行 Outlook 的詳細指引，請參閱 < <b0>Configure 監督原則</b0>。
...
在某些情況下，您需要提供資訊給法規或規範稽核員證明監督的員工活動和通訊。 這可能是相關聯的已定義的原則或只要監督原則是變更或更新的所有主管活動的摘要。 監督原則必須為內部或外部稽核完成整備的內建的稽核線索。 證明主管的程序會示範監視監督原則的每個動作的詳細的稽核歷程記錄。
