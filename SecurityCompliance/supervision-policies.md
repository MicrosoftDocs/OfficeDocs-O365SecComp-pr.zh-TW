...
您將建立監督原則與合規性中心。 這些原則定義哪些通訊和使用者會受到檢閱您組織中，定義自訂條件通訊必須符合，並指定誰應該執行的檢閱。 主管檢閱角色群組中包含的使用者可以設定原則，並擁有此角色指派給任何人可以存取合規性中心中的 [監督] 頁面。
...
當您建立的監督原則時，您也將決定誰可以執行檢閱的監督使用者的郵件。 在原則中，您將使用使用者電子郵件地址來識別的個人或群組的人員檢閱監督的通訊。 所有檢閱者必須擁有 Exchange Online 上主控的信箱。
...
如果您想要減少要檢閱的內容量，您可以指定百分比由監督原則的所有通訊。 我們隨機將符合您所選擇的條件的總百分比從選取內容的量。 如果您想要檢閱者的所有項目，您可以輸入監督原則中的**100%** 。
...
若要使用的監督原則儀表板是最簡單的方法來管理監督原則結果，以及解決待處理項目。 這個儀表板可以讓檢閱者若要快速查看需要檢閱、 採取動作的項目，並檢視先前的結果項目檢閱並解決每一個監督原則的項目。 您可以存取合規性中心，在**監督**監督原則儀表板 > *您的自訂原則* > **開啟**。
...
使用者指定作為監督原則中的檢閱者可以使用網頁型 Outlook 來檢閱並解決監督項目。 監督增益集會自動安裝在網頁型 Outlook 原則中指定的所有檢閱者。 監督原則共用資料夾，以供設定的檢閱者不需要任何額外的設定。

網頁型 Outlook，檢閱者可以進行下列作業：
...
當監督原則識別通訊郵件進行檢閱時，電子郵件會傳遞至 Outlook 和 Outlook 網頁 （原先稱為 Outlook Web App） 中的檢閱者監督資料夾。 這份報告列出每個原則，以及通訊數目，每個階段中檢閱程序。
...
整合的稽核記錄搜尋函數或[Search-unifiedauditlog](https://docs.microsoft.com/powershell/module/exchange/policy-and-compliance-audit/search-unifiedauditlog) PowerShell cmdlet 檢視稽核活動。
