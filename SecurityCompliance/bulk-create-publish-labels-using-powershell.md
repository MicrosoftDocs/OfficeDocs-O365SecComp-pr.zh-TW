...
ms.locfileid: "34152105"
---
# <a name="bulk-create-and-publish-retention-labels-by-using-powershell"></a><span data-ttu-id="33546-108">使用 PowerShell 大量建立及發佈保留標籤</span><span class="sxs-lookup"><span data-stu-id="33546-108">Bulk create and publish retention labels by using PowerShell</span></span>

<span data-ttu-id="33546-p102">在 Office 365 中，您可以使用標籤來為您的組織實作保留排程。做為記錄管理員或合規性主管，您可能有數百個要建立及發佈的標籤。您可以透過安全性與合規性中心的 UI 來執行此動作，但一次建立一個標籤費時且無效率。</span><span class="sxs-lookup"><span data-stu-id="33546-p102">In Office 365, you can use labels to implement a retention schedule for your organization. As a record manager or compliance officer, you might have hundreds of labels to create and publish. You can do this through the UI in the Security &amp; Compliance Center, but creating labels one at a time is time-consuming and inefficient.</span></span>
  
<span data-ttu-id="33546-p103">使用下面提供的指令碼和 .csv 檔案，您可以大量建立及發佈標籤與標籤原則。首先在 Excel 中建立標籤清單以及標籤原則清單，然後使用 PowerShell ，在這些清單中大量建立標籤及標籤原則。這可讓您更輕鬆地一次建立您的保留排程所需的所有標籤。</span><span class="sxs-lookup"><span data-stu-id="33546-p103">By using the script and .csv files provided below, you can bulk create and publish labels and label policies. First you create a list of the labels and a list of the label policies in Excel, and then you bulk create the labels and label policies in those lists by using PowerShell. This makes it easier to create and publish at one time all of the labels that your retention schedule requires.</span></span>
  
<span data-ttu-id="33546-115">如需標籤的詳細資訊，請參閱[標籤概觀](labels.md)。</span><span class="sxs-lookup"><span data-stu-id="33546-115">For more information about labels, see [Overview of labels](labels.md).</span></span>
  
## <a name="disclaimer"></a><span data-ttu-id="33546-116">免責聲明</span><span class="sxs-lookup"><span data-stu-id="33546-116">Disclaimer</span></span>

<span data-ttu-id="33546-p104">在任何 Microsoft 標準支援程式或服務下，不支援本主題提供的指令碼。範例指令碼係依「現狀」提供，不附帶任何明示或默示的擔保。Microsoft 另外不承擔任何明示或默示的擔保，包括但不限於適售性或適合某特定用途的默示擔保。使用或操作範例指令碼和文件發生的所有風險皆屬於您的責任。Microsoft、其作者以及其他與建置、生產或交付程式碼相關的任何人在任何情況下皆完全不需對任何損失負責任，包括但不限於商業利潤損失、業務中斷、業務資訊損失、或其他錢財損失等因使用或無法使用範例指令碼所發生的損失，即使 Microsoft 曾建議這些損失發生的可能性。</span><span class="sxs-lookup"><span data-stu-id="33546-p104">The sample scripts provided in this topic aren't supported under any Microsoft standard support program or service. The sample scripts are provided AS IS without warranty of any kind. Microsoft further disclaims all implied warranties including, without limitation, any implied warranties of merchantability or of fitness for a particular purpose. The entire risk arising out of the use or performance of the sample scripts and documentation remains with you. In no event shall Microsoft, its authors, or anyone else involved in the creation, production, or delivery of the scripts be liable for any damages whatsoever (including, without limitation, damages for loss of business profits, business interruption, loss of business information, or other pecuniary loss) arising out of the use of or inability to use the sample scripts or documentation, even if Microsoft has been advised of the possibility of such damages.</span></span>
  
## <a name="step-1-create-a-csv-file-for-creating-the-labels"></a><span data-ttu-id="33546-122">步驟 1：建立 .csv 檔案以建立標籤</span><span class="sxs-lookup"><span data-stu-id="33546-122">Step 1: Create a .csv file for creating the labels</span></span>

<span data-ttu-id="33546-p105">首先，建立一個 .csv 檔案，其中包含標籤清單及標籤的保留設定。您可以使用下列範例做為範本，方法為將它複製到 Excel，同時將文字轉換為資料行 (在 Excel \> [資料]\*\*\*\* 索引標籤 \> [文字到資料行]\*\*\*\* \> [分隔符號]\*\*\*\* \> [逗號]\*\*\*\*\> [一般]\*\*\*\* 中)，然後在易於找到的位置中將工作表儲存為 .csv 檔案。</span><span class="sxs-lookup"><span data-stu-id="33546-p105">First you create a .csv file that contains a list of your labels with their retention settings. You can use the sample below as a template by copying it into Excel, converting the text to columns (in Excel \> **Data** tab \> **Text to Columns** \> **Delimited** \> **Comma** \> **General**), and then saving the worksheet as a .csv file in a location that's easy to find.</span></span>
  
<span data-ttu-id="33546-125">如需此 Cmdlet 的參數值的詳細資訊，請參閱 [New-ComplianceTag](https://go.microsoft.com/fwlink/?linkid=866511)。</span><span class="sxs-lookup"><span data-stu-id="33546-125">For more information about the parameter values for this cmdlet, see [New-ComplianceTag](https://go.microsoft.com/fwlink/?linkid=866511).</span></span>
  
<span data-ttu-id="33546-126">附註：</span><span class="sxs-lookup"><span data-stu-id="33546-126">Notes:</span></span>
  
- <span data-ttu-id="33546-127">如果您沒有提供來源檔案來建立標籤，則指令碼會繼續進行，並提示您提供來源檔案來發佈標籤 (請參閱下一節)，而且指令碼只會發佈現有的標籤。</span><span class="sxs-lookup"><span data-stu-id="33546-127">If you don't provide a source file for creating labels, the script moves on and prompts you for the source file for publishing labels (see the next section), and the script will publish only existing labels.</span></span>
    
- <span data-ttu-id="33546-p106">如果 .csv 檔案包含的標籤與已存在的標籤同名，則指令碼會略過建立該標籤。不會建立重複的標籤。</span><span class="sxs-lookup"><span data-stu-id="33546-p106">If the .csv file contains a label with the same name as one that already exists, the script skips creating that label. No duplicate labels are created.</span></span>
    
- <span data-ttu-id="33546-p107">如果您變更或重新命名資料行標頭，則指令碼將失敗。指令碼需要 .csv 檔案具有下列提供的格式。</span><span class="sxs-lookup"><span data-stu-id="33546-p107">If you change or rename the column headers, the script will fail. The script requires a .csv file in the format provided here.</span></span>
    
### <a name="sample-csv-file"></a><span data-ttu-id="33546-132">範例 .csv 檔案</span><span class="sxs-lookup"><span data-stu-id="33546-132">Sample .csv file</span></span>
...
## <a name="step-2-create-a-csv-file-for-publishing-the-labels"></a><span data-ttu-id="33546-133">步驟 2：建立 .csv 檔案以發佈標籤</span><span class="sxs-lookup"><span data-stu-id="33546-133">Step 2: Create a .csv file for publishing the labels</span></span>

<span data-ttu-id="33546-p108">接著，建立一個 .csv 檔案，其中包含標籤原則清單以及標籤原則的位置及其他設定。您可以使用下列範例做為範本，方法為將它複製到 Excel，同時將文字轉換為資料行 (在 Excel \> [資料]\*\*\*\* 索引標籤 \> [文字到資料行]\*\*\*\* \> [分隔符號]\*\*\*\* \> [逗號]\*\*\*\*\> [一般]\*\*\*\* 中)，然後在易於找到的位置中將工作表儲存為 .csv 檔案。</span><span class="sxs-lookup"><span data-stu-id="33546-p108">Next you create a .csv file that contains a list of label policies with their locations and other settings. You can use the sample below as a template by copying it into Excel, converting the text to columns (in Excel \> **Data** tab \> **Text to Columns** \> **Delimited** \> **Comma** \> **General**), and then saving the worksheet as a .csv file in a location that's easy to find.</span></span>
  
<span data-ttu-id="33546-136">如需此 Cmdlet 的參數值的詳細資訊，請參閱 [New-RetentionCompliancePolicy](https://go.microsoft.com/fwlink/?linkid=866512)。</span><span class="sxs-lookup"><span data-stu-id="33546-136">For more information about the parameter values for this cmdlet, see [New-RetentionCompliancePolicy](https://go.microsoft.com/fwlink/?linkid=866512).</span></span>
  
<span data-ttu-id="33546-137">附註：</span><span class="sxs-lookup"><span data-stu-id="33546-137">Notes:</span></span>
  
- <span data-ttu-id="33546-138">如果您沒有提供來源檔案來發佈標籤，則指令碼會建立標籤 (請參閱前一節)，但不會發佈它們。</span><span class="sxs-lookup"><span data-stu-id="33546-138">If you don't provide a source file for publishing labels, the script creates labels (see the previous section) but does not publish them.</span></span>
    
- <span data-ttu-id="33546-p109">如果 .csv 檔案包含的標籤原則與已存在的標籤原則同名，則指令碼會略過建立該標籤原則。不會建立重複的標籤原則。</span><span class="sxs-lookup"><span data-stu-id="33546-p109">If the .csv file contains a label policy with the same name as one that already exists, the script skips creating that label policy. No duplicate label policies are created.</span></span>
    
- <span data-ttu-id="33546-p110">指令碼只會發佈手動套用至內容的標籤。此指令碼不支援自動套用至內容的標籤。</span><span class="sxs-lookup"><span data-stu-id="33546-p110">The script publishes only labels that are applied manually to content. This script does not support labels that are auto-applied to content.</span></span>
    
- <span data-ttu-id="33546-p111">如果您變更或重新命名資料行標頭，則指令碼將失敗。指令碼需要 .csv 檔案具有下列提供的格式。</span><span class="sxs-lookup"><span data-stu-id="33546-p111">If you change or rename the column headers, the script will fail. The script requires a .csv file in the format provided here.</span></span>
    
### <a name="sample-csv-file"></a><span data-ttu-id="33546-145">範例 .csv 檔案</span><span class="sxs-lookup"><span data-stu-id="33546-145">Sample .csv file</span></span>
...
## <a name="step-3-create-the-powershell-script"></a><span data-ttu-id="33546-146">步驟 3：建立 PowerShell 指令碼</span><span class="sxs-lookup"><span data-stu-id="33546-146">Step 3: Create the PowerShell script</span></span>

<span data-ttu-id="33546-p112">複製下列 PowerShell 指令碼，並將其貼入 [記事本]。使用檔案名稱尾碼 .ps1，將檔案儲存在易於找到的位置，例如，\<path\>CreateRetentionSchedule.ps1。</span><span class="sxs-lookup"><span data-stu-id="33546-p112">Copy and paste the below PowerShell script into Notepad. Save the file by using a filename suffix of .ps1 in a location that's easy to find -- for example, \<path\>CreateRetentionSchedule.ps1.</span></span>
  
### <a name="powershell-script"></a><span data-ttu-id="33546-149">PowerShell 指令碼</span><span class="sxs-lookup"><span data-stu-id="33546-149">PowerShell script</span></span>
...
## <a name="step-4-connect-to-security-amp-compliance-center-powershell"></a><span data-ttu-id="33546-150">步驟 4︰連接到安全性與合規性中心 PowerShell</span><span class="sxs-lookup"><span data-stu-id="33546-150">Step 4: Connect to Security &amp; Compliance Center PowerShell</span></span>

<span data-ttu-id="33546-151">請遵循下列步驟：</span><span class="sxs-lookup"><span data-stu-id="33546-151">Follow the steps here:</span></span>
  
- [<span data-ttu-id="33546-152">連接到 Office 365 安全性與合規性中心 PowerShell</span><span class="sxs-lookup"><span data-stu-id="33546-152">Connect to Office 365 Security &amp; Compliance Center PowerShell</span></span>](https://go.microsoft.com/fwlink/?linkid=799771)
    
## <a name="step-5-run-the-powershell-script-to-create-and-publish-the-labels"></a><span data-ttu-id="33546-153">步驟 5：執行 PowerShell 指令碼來建立及發佈標籤</span><span class="sxs-lookup"><span data-stu-id="33546-153">Step 5: Run the PowerShell script to create and publish the labels</span></span>

<span data-ttu-id="33546-154">在您已連接到安全性與合規性中心 PowerShell 之後，接著執行指令碼來建立及發佈標籤。</span><span class="sxs-lookup"><span data-stu-id="33546-154">After you've connected to Security &amp; Compliance Center PowerShell, next you run the script that creates and publishes the labels.</span></span>
  
1. <span data-ttu-id="33546-155">在安全性與合規性中心 PowerShell 工作階段中，輸入路徑，後面跟著字元 .\ 和指令碼的檔案名稱，然後按 ENTER 以執行指令碼 - 例如：</span><span class="sxs-lookup"><span data-stu-id="33546-155">In the Security &amp; Compliance PowerShell session, enter the path, followed by the characters .\ and file name of the script, and then press ENTER to run the script - for example:</span></span>
    
  ```
  <path>.\CreateRetentionSchedule.ps1
  ```

    <span data-ttu-id="33546-156">指令碼會提示您輸入您先前所建立之 .csv 檔案的位置。</span><span class="sxs-lookup"><span data-stu-id="33546-156">The script will prompt you for the locations of the .csv files that you created above.</span></span>
    
2. <span data-ttu-id="33546-157">輸入路徑，後面跟著字元 .\ 和 .csv 檔案的檔案名稱，然後按 ENTER - 例如：</span><span class="sxs-lookup"><span data-stu-id="33546-157">Enter the path, followed by the characters .\ and file name of the .csv file, and then press ENTER - for example:</span></span>
    
  ```
  <path>.\LabelsToCreate.csv
  ```

## <a name="step-6-view-the-log-file-with-the-results"></a><span data-ttu-id="33546-158">步驟 6：檢視記錄檔與結果</span><span class="sxs-lookup"><span data-stu-id="33546-158">Step 6: View the log file with the results</span></span>

<span data-ttu-id="33546-p113">當您執行指令碼時，它會產生記錄檔，記錄其已採取的每個動作，以及動作成功還是失敗。記錄檔包含有關已建立哪些標籤和已發佈哪些標籤的所有中繼資料。您可以在此位置尋找記錄檔 -- 請注意，檔案名稱中的數字有所不同。</span><span class="sxs-lookup"><span data-stu-id="33546-p113">When you run the script, it generates a log file that records each action it took and whether the action succeeded or failed. The log file includes all metadata about what labels were created and what labels were published. You can find the log file at this location -- note that the digits in the file name vary.</span></span>
