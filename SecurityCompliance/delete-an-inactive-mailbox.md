...
在前任員工離開您的組織之後，可透過非使用中信箱來保留其電子郵件。 當您不再需要保留非使用中信箱的內容時，您可以永久刪除非使用中信箱移除保留。 此外，很可能多項保留可能處於非使用中的信箱。 例如，非使用中信箱也可能會放置訴訟暫止，並在一或多個就地保留。 此外，Office 365 保留原則 (建立 Office 365 安全性&amp;合規性中心) 可能會套用至非使用中信箱。 您必須將它刪除非使用中信箱中移除所有的保留和 Office 365 保留原則。 保留和保留原則中移除之後，在作用中信箱標記為待刪除，並永久刪除之後就會處理。
