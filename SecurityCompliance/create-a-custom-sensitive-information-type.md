...
但是如果您需要識別及保護不同類型的機密資訊 (例如使用貴組織專屬格式的員工識別碼)，可以建立自訂機密資訊類型。機密資訊類型是在稱為規則套件的 XML 檔案中定義。
...
2. [使用遠端 PowerShell 連線到 Office 365 安全性與合規性中心](http://go.microsoft.com/fwlink/?LinkID=799771&amp;clcid=0x409)
...
5. 藉由輸入 Get-DlpSensitiveInformationType 以查看所有機密類型的清單，確認您的新機密資訊類型已上傳。您可以藉由查看 [發行者] 資料行，快速地分隔自訂機密資訊類型與內建的類型。您可以藉由執行 Get-DlpSensitiveInformationType -Identity "name of sensitive information type"，將清單篩選為特定機密資訊類型。
...
在 Office 365 中，您無法手動要求對整個租用戶重新編目，但是您可以為網站集合、清單或文件庫這麼做，請參閱[手動要求網站、文件庫或清單的編目和重新編製索引](https://support.office.com/article/9afa977d-39de-4321-b4ca-8c7c7e6d264e) (機器翻譯)。
...
2. 在安全性與合規性中心 PowerShell 中，執行下列其中一個動作：
    
  - 若要移除整個規則套件及其包含的所有實體
    
    輸入 Remove-DlpSensitiveInformationTypeRulePackage "NameOfYourRulePack" - 針對上述範例 XML，您要輸入 Remove-DlpSensitiveInformationTypeRulePackage "Employee ID Custom Rule Pack"。
    
    請注意，若要識別您的規則套件，可以在 \<Rule Pack\> 元素或 RulePack 元素之 id 屬性的 GUID 中，使用 \<Name\> 元素 (適用於任何語言)。
    
  - 若要從規則套件中移除單一實體
    
    您必須使用 Set-DlpSensitiveInformationTypeRulePackage，在該特定實體已移除的請況下，上傳新版本的規則套件。您必須在移除之前，確定沒有 DLP 原則或 Exchange 傳輸規則仍在參考機密資訊類型。
    
3. 若要確認，輸入 Y，並按下 ENTER 鍵。
    
4. 藉由輸入 Get- DlpSensitiveInformationType 來確認您的新規則已移除，應該不會再顯示您的機密資訊類型的名稱。
